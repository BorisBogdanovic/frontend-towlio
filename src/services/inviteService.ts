...
export const checkInviteToken = async (
  ...
  try {
    const response = await fetch(`${API_URL}/invites/validate/${token}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (err instanceof Error) {
      throw new Error(err.message || "Unexpected error checking invite token");
    }
    throw new Error("Unknown error checking invite token");
  }
};
///////////////////////////////////////////////////////////////////////// RESEND INVITE
export async function resendInvite(email: string): Promise<InviteResponse> {
  const authToken = getAuthToken();
  if (!authToken) throw new Error("User not authenticated");
  try {
    const response = await fetch(`${API_URL}/invites/resend`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${authToken}`,
